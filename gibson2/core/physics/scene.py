...
class GraspScene(Scene):
    """
    A simple grasp scene for debugging
    """
    def load(self):
        self.build_graph = False
        self.is_interactive = False
        ...
        planeName = os.path.join(pybullet_data.getDataPath(), "mjcf/ground_plane.xml")
        self.ground = p.loadMJCF(planeName)[0]
        filename = os.path.join(pybullet_data.getDataPath(), "table/table.urdf")
        self.table = p.loadURDF(filename, flags=p.URDF_USE_MATERIAL_COLORS_FROM_MTL)
        ...
        # return [self.ground] + [self.table]


        return list(self.stadium) + [self.ground] + [self.table]
